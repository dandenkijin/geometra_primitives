# Compiler Full — PRD
## Goal
Build a full compiler pipeline that takes .pga syntax, parses via PgaAst (AST intermediate), emits exact WGGL shader strings, supports multi-backend slots.
## In Scope
- Parser (.pga tokenizer + AST builder)
- Emission layer (WGGL shader strings with exact geometric contracts)
...
- Emitted WGGL strings match geometric contracts (wedge, vee, sandwich, primitives)
